...
import com.dongbingbin.app.mapboxdemo.AppConfig;
...
public class LogUtil {
    ...
    public static final String TAG_ZXJ = "TAG_ZXJ";
    ...
    private static final String BASE_DIR = AppApplication.getAppApplication().getExternalCacheDir()
            .getAbsolutePath() + File.separator + "jjcrash";
    ...
    public synchronized static void writeLog(byte log[]) {
        ...
        for (int i = 0; i < log.length; i++) {
            slog += " " + LogUtil.toHex(log[i]);
        }
        LogUtil.writeLogFile(slog);
    }
...
//    }
    ...
    public synchronized static void buildLogMessage(Throwable ex, Context context) {
        StringBuilder builder = new StringBuilder();
        if (ex != null) {
            builder.append("\n\n").append("------ 程序错误信息 -------").append("\n")
                    .append(LogUtil.getLogStyle(LogUtil.ERROR, TAG, "", ex));
        }
        long currentTime = System.currentTimeMillis();
